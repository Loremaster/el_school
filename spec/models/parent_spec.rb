  ...
  describe "Validations" do
    describe "Rejection" do
      ...
      it "should reject first name with digits" do
        wrong_attr = @attr_parent.merge( :parent_first_name => 'asd2' )
        @user.build_parent( wrong_attr ).should_not be_valid
      end

      it "should reject first name with space" do
        wrong_attr = @attr_parent.merge( :parent_first_name => 'asd ' )
        @user.build_parent( wrong_attr ).should_not be_valid
      end

      it "should reject last name with digits" do
        wrong_attr = @attr_parent.merge( :parent_last_name => 'asd2' )
        @user.build_parent( wrong_attr ).should_not be_valid
      end

      it "should reject last name with space" do
        wrong_attr = @attr_parent.merge( :parent_last_name => 'asd ' )
        @user.build_parent( wrong_attr ).should_not be_valid
      end

      it "should reject middle name with digits" do
        wrong_attr = @attr_parent.merge( :parent_middle_name => 'asd2' )
        @user.build_parent( wrong_attr ).should_not be_valid
      end

      it "should reject middle name with space" do
        wrong_attr = @attr_parent.merge( :parent_middle_name => 'asd ' )
        @user.build_parent( wrong_attr ).should_not be_valid
      end
    end
