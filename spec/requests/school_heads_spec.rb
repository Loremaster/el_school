...
describe "SchoolHeads" do
  ...
  describe "for sign-in school heads" do
    ...
    describe "Events" do
      ...
      it "should show created event" do
        click_link "Мероприятия"
        click_link "Показать мероприятия для класса"
        click_link @event.school_class.class_code

        response.should have_selector("tbody") do |tbody|
          tbody.should have_selector("tr") do |tr|
            tr.should have_selector( 'td',  :content => @event.event_place )
            tr.should have_selector( 'td',  :content => @event.event_place_of_start )
            tr.should have_selector( 'td',  :content => @event.event_begin_date.strftime("%d.%m.%Y") )
            tr.should have_selector( 'td',  :content => @event.event_end_date.strftime("%d.%m.%Y") )
            tr.should have_selector( 'td',  :content => @event.event_begin_time.strftime("%H:%M") )
            tr.should have_selector( 'td',  :content => @event.event_end_time.strftime("%H:%M") )
            tr.should have_selector( 'td',  :content => "#{@event.event_cost}" )
          end
        end
      end
    end
